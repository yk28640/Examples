...
namespace OpennessServices
{
    ...
    public class TiaOpennessClient : IDisposable
    {
        ...
        public PlcSoftware GetPlcSoftware(string deviceName)
        {
           ...
           return device.DeviceItems[1].GetService<SoftwareContainer>()?.Software as PlcSoftware;
        ...
        public OnlineCompareSummary ComparePlcProgramOnline( String projectPath)
        {
            ...
            //CompareResult result = GoOnlineAndCompare(device);
            //WriteResultOffical(result.RootElement, string.Empty);;
            ...
            var knowHowProtectedBlockNames = plcsoftware.BlockGroup.Blocks
                                                .Where(b => b.IsKnowHowProtected)
                                                .Select(b => b.Name)
                                                .ToList();

            foreach (var blockName in knowHowProtectedBlockNames)
            {
                Console.WriteLine($"块名: {blockName}");
            }


            SetConnectionWithSlot(tiaPortal.Projects[0].Devices[4].DeviceItems[1].GetService<OnlineProvider>());
